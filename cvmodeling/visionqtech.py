...
model_path = r"best_model_0.002221.pt"  # 모델 경로
p_scr = r"pruned_scripted_model.pt"
...
run_once_flag = True
stop_once_flag = False

def modbus_read_val():
    global count, Good_Count, Bad_Count, Converyor_Status
    global run_once_flag, stop_once_flag

    start_signal = server.data_bank.get_holding_registers(20)
    count_reset = server.data_bank.get_holding_registers(21)
    good_reset = server.data_bank.get_holding_registers(22)
    bad_reset = server.data_bank.get_holding_registers(23)
    light_state = server.data_bank.get_holding_registers(8)

    if (start_signal[0] == 1) and (run_once_flag == True):
        Converyor_Status = 1
        send_conveyor_speed(255)
        stop_once_flag = True
        run_once_flag = False
    if (start_signal[0] == 0) and (stop_once_flag == True):
        Converyor_Status = 0
    ...
    if count_reset[0] == 1:
        count = 0
        server.data_bank.set_holding_registers(21, [0])

    if good_reset[0] == 1:
        Good_Count = 0
        server.data_bank.set_holding_registers(22, [0])
    
    if bad_reset[0] == 1:
        Bad_Count = 0
        server.data_bank.set_holding_registers(23, [0])

    if light_state[0] == 1:
        server.data_bank.set_holding_registers(8, [0])
        server.data_bank.set_holding_registers(9, [3])
        ser.write("LAMP_RED=OFF\n".encode())
        ser.write("LAMP_YELLOW=OFF\n".encode())
        ser.write("LAMP_GREEN=ON\n".encode())
        Good_Count += 1
    elif light_state[0] == 2:
        server.data_bank.set_holding_registers(8, [0])
        server.data_bank.set_holding_registers(9, [2])
        ser.write("LAMP_RED=OFF\n".encode())
        ser.write("LAMP_YELLOW=ON\n".encode())
        ser.write("LAMP_GREEN=OFF\n".encode())
    elif light_state[0] == 3:
        server.data_bank.set_holding_registers(8, [0])
    ...
    threading.Timer(0.5, modbus_read_val).start()
...
t3 = threading.Thread(target=modbus_read_val)
t3.daemon =True
t3.start()
